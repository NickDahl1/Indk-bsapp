...
function ShoppingListPage() {
  ...
  const groupedData = useMemo(() => {
    ...
    const storesData = MAIN_STORES.map(storeName => {
      ...
      // Filter out completed items in shopping mode
      if (isShoppingMode) {
        storeItems = storeItems.filter(item => !item.isCompleted);
      }
      
      // If no items in this store and we are in shopping mode, skip this store
      if (isShoppingMode && storeItems.length === 0) {
        return null;
      }
      ...
      // Always sort items by category order
      const sortedItems = [...storeItems].sort((a, b) => {
        const catA = a.category || 'Andet';
        const catB = b.category || 'Andet';
      ...
      if (showCategories) {
        ...
        return { storeName, showCategories, categories, items: sortedItems };
      } else {
        return { storeName, showCategories, categories: [], items: sortedItems };
      }
    ...
    let otherItems = items.filter(item => !item.store || !MAIN_STORES.includes(item.store as any));
    
    if (isShoppingMode) {
      otherItems = otherItems.filter(item => !item.isCompleted);
    }
    ...
    return storesData;
  }, [items, storeCategoryToggles, isShoppingMode]);
  ...
  return (
    <div className={cn("p-4 sm:p-8 max-w-5xl mx-auto transition-all duration-500", isShoppingMode && "bg-gray-50 min-h-screen")}>
      ...
      {/* Shopping Mode Header Stats */}
      {isShoppingMode && items.length > 0 && (
        <div className="mb-8 p-6 bg-gray-900 rounded-[24px] text-white flex items-center justify-between shadow-xl animate-in fade-in slide-in-from-top-4">
          <div>
            <p className="text-gray-400 text-[10px] sm:text-xs font-bold uppercase tracking-widest mb-1">Varer tilbage</p>
            <p className="text-2xl sm:text-3xl font-light">{items.filter(i => !i.isCompleted).length}</p>
          </div>
          ...
          <div className="text-right ml-4">
            <p className="text-gray-400 text-[10px] sm:text-xs font-bold uppercase tracking-widest mb-1">Færdige</p>
            <p className="text-2xl sm:text-3xl font-light text-emerald-400">{items.filter(i => i.isCompleted).length}</p>
          </div>
        </div>
      )}
...
function AppContent() {
  ...
  const [activeTab, setActiveTab] = useState<'indkob' | 'statistik'>('indkob');
